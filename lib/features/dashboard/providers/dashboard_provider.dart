...
class DashboardNotifier extends AsyncNotifier<DashboardState> {
  ...
  Future<DashboardState> _loadData(DashboardPeriod period) async {
    ...
    try {
      ...
      if (period == DashboardPeriod.sevenDays) {
        ...
        entireStart = DateTime(startOfWeek.year, startOfWeek.month, startOfWeek.day, 0, 0, 0);
      } else {
      ...
      for (int i = loopCount - 1; i >= 0; i--) {
        ...
        final rangeTxs = allRangeTxs.where((tx) =>
            tx.createdAt.isAfter(startRange.subtract(const Duration(seconds: 1))) &&
        ...
        final rangeCosts = allRangeCosts.where((c) =>
        ...
        final rangeReceivables = allReceivables.where((r) =>
            r.createdAt.isAfter(startRange.subtract(const Duration(seconds: 1))) &&
            r.createdAt.isBefore(endRange.add(const Duration(seconds: 1))));
        final dayReceivables = rangeReceivables.fold<int>(0, (sum, r) => sum + r.amount);
  ...
  Future<void> changePeriod(DashboardPeriod period) async {
    ...
    if (currentState != null) {
      state = await AsyncValue.guard(() => _loadChartTrendOnly(period, currentState));
    } else {
  ...
  Future<void> reload() async {
    final currentPeriod = state.valueOrNull?.selectedPeriod ?? DashboardPeriod.sevenDays;
